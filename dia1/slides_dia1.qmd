...
MINICURSO_ALVO = "inteligencia artificial em financas"
...
def localizar_planilha():
    ...
    for base in bases:
        arquivos = sorted(base.glob("Inscritos - Escola SBFin*.xlsx"))
        if arquivos:
            return arquivos[0]

    for base in bases:
        arquivos = sorted(path for path in base.glob("*.xlsx") if not path.name.startswith("~$"))
        if arquivos:
            return arquivos[0]

    raise FileNotFoundError("Planilha Excel não encontrada na pasta do projeto ou no diretório pai.")
...
colunas_ausentes = [coluna for coluna in COLUNAS_PERFIL if coluna not in df.columns]
if colunas_ausentes:
    raise ValueError(
        "A planilha de inscritos não contém as colunas esperadas: "
        + ", ".join(colunas_ausentes)
    )

minicursos = df["Minicurso"].fillna("").astype(str).map(remover_acentos).str.lower()
df = df.loc[minicursos == MINICURSO_ALVO].copy()

if df.empty:
    raise ValueError("Nenhuma inscrição encontrada para o minicurso de IA aplicada a Finanças.")
...
def normalizar_curso(valor):
    ...
    texto_min = remover_acentos(texto).lower()

    if texto_min in {"", "-", "nao informado", "nan"}:
        return "Não informado"
    if "cont" in texto_min:
        return "Ciências Contábeis"
    if "comput" in texto_min or "sistema" in texto_min or "desenvolvimento de sistemas" in texto_min:
...
![](images/QR_code.png){fig-align="center" width="18%"}
...
![](images/github_repo_QR.png){fig-align="center" width="18%"}
...
![](images/claude_prompt_curto.png){fig-align="center" width="80%"}

## Resultado: Muito Abrangente
![](images/claude_aula_valuation.png){fig-align="center" width="90%"}

## Prompt Bom em Ação
![](images/claude_prompt_longo.png){fig-align="center" width="80%"}

## Resultado: Foco na Tarefa
![](images/claude_calculo_impacto.png){fig-align="center" width="90%"}

## Resultado: Foco Até em Limitações
![](images/claude_limitacoes.png){fig-align="center" width="99%"}
...
![](images/tabela_portfolios_claude.png){fig-align="center"}

## Limitações
![](images/limitacoes_claude.png){fig-align="center" width="70%"}
...
![](images/tabela_backtest.png){fig-align="center"}

## Evolução da Estratégia no Backtest
![](images/backtest_drawdown.png){fig-align="center" width="80%"}

## Recomendação
![](images/recomendacao.png){fig-align="center" width="80%"}
